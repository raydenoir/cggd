...
namespace cg::renderer
{
	...
	inline void rasterizer<VB, RT>::draw(size_t num_vertexes, size_t vertex_offset)
	{
		...
		while (vertex_id < vertex_offset + num_vertexes) {
			...
			int2 bb_begin = clamp(min_vertex, int2{0, 0}, int2{static_cast<int>(width) - 1, static_cast<int>(height) - 1});

			int2 max_vertex = max(vertex_a, max(vertex_b, vertex_c));
			int2 bb_end = clamp(max_vertex, int2{0, 0}, int2{static_cast<int>(width) - 1, static_cast<int>(height) - 1});
